...
<body>
  ...
  <h1>.</h1>
  <table>
    <caption>Tabela de Reservas</caption>
    <caption class="addLivro"><a href="tabelaReservaAnalise.php"><button type="button" class="btn">Reservas Pendentes <i class="fas fa-eye"></i></button></a></caption>
    <thead>
      <tr>
        <th>RM</th>
        <th>Nome do Aluno</th>
    ...
    <?php
      include('../conn.php');
      $results_per_page = 25;
      $sql = 'SELECT * FROM reservanalise WHERE Confirm = 1';
      ...
      $sql = 'SELECT * FROM reservanalise WHERE Confirm = 1 LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
      $result = mysqli_query($conn, $sql);
      while ($linha = mysqli_fetch_array($result)) {
        echo '  <tr>
                  <td>' . $linha['RM'] . '</td>
                  <td>' . $linha['Nome'] . '</td>
                  <td>' . $linha['Cod_livro'] . '</td>
                  <td>' . $linha['Livro'] . '</td>
                  <td>' . $linha['Emprestimo'] . '</td>
                  <td>' . $linha['Devolucao'] . '</td>
                  <td><a onclick="exclui('.$linha['Cod'].')" href="#"><i class="fas fa-times"></a></td>
                  <td><a onclick="lembrar('.$linha['Cod'].')" href="#"><i class="fas fa-envelope"></a></td>
                </tr>';
      };
      echo '</table>';
      echo '<div class="pagination">';
      for ($page = 1; $page <= $number_of_pages; $page++) {
        echo '<a href="tabelaReservas.php?page=' . $page . '" class="btn">' . $page . '</a>';
      }
      echo '</div>';
    ?>
  ...
  <script>
    function exclui(cod){
      if(confirm("Deseja Devolver o Pedido com Código " + cod + "?")){
        location.href='../../FUNCIONALIDADES/ADM/delete.php?cod='+cod+'&delete=devol';
      }
    }
    function lembrar(cod){
      if(confirm("Deseja lembrar do Prazo para o Aluno com o RM " + cod + "?")){
        location.href='../../FUNCIONALIDADES/ADM/email.php?rm=' + cod+'&verif=aviso';
      }
    }
  </script>
</body>
