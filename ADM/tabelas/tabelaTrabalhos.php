...
<body>
  ...
  <h1>.</h1>
  <table>
    <caption>Tabela de Trabalhos</caption>
    <caption class="addLivro"><a href="../adicionar/adicionarTrabalho.php"><button type="button" class="btn">Adicionar Trabalho <i class="fas fa-plus-circle"></i></button></a></caption>
    <thead>
      <tr>
        <th>Cód</th>
        <th>Titulo</th>
    ...
    <?php
      ...
      $sql = 'SELECT * FROM trabalhos LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
      $result = mysqli_query($conn, $sql);
      while ($linha = mysqli_fetch_array($result)) {
        echo '  <tr>
                  <td>' . $linha['Cod'] . '</td>
                  <td>' . $linha['Nome_Trab'] . '</td>
                  <td>' . $linha['Autor_Trab'] . '</td>
                  <td>' . $linha['Tipo_Trab'] . '</td>
                  <td>' . $linha['Ano_Trab'] . '</td>
                  <td><a href="../alterar/alterarTrabalho.php?codigo=' . $linha['Cod'] . '"><i class="far fa-edit"></a></td>
                  <td><a href="../visualizar/visualizarTrabalho.php?codigo=' . $linha['Cod'] . '"><i class="fas fa-eye"></a></td>
      ...
      for ($page = 1; $page <= $number_of_pages; $page++) {
        echo '<a href="tabelaTrabalhos.php?page=' . $page . '" class="btn">' . $page . '</a>';
      }
      echo '</div>';
    ?>
  ...
  <script>
    function exclui(cod){
      if(confirm("Deseja excluir o Trabalho com Código " + cod + "?")){
        location.href='../../FUNCIONALIDADES/ADM/delete.php?cod=' + cod + '&delete=trabalho';
      }
    }
  </script>
</body>
