...
<body>
  ...
  <h1>.</h1>
  <table>
    <caption>Tabela de Sugestões</caption>
    <thead>
      <tr>
        <th>Cód</th>
        <th>RM</th>
    ...
    <?php
      ...
      $sql = 'SELECT * FROM sugestao LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
      $result = mysqli_query($conn, $sql);
      while ($linha = mysqli_fetch_array($result)) {
        echo '  <tr>
                  ...
                  <td>' . $linha['Autor_Livro'] . '</td>
                  <td><a href="../visualizar/visualizarSugestao.php?codigo=' . $linha['Cod'] . '"><i class="fas fa-eye"></a></td>
      ...
      for ($page = 1; $page <= $number_of_pages; $page++) {
        echo '<a href="tabelaSugestões.php?page=' . $page . '" class="btn">' . $page . '</a>';
      }
      echo '</div>';
    ?>
  ...
  <script>
    function exclui(cod){
      if(confirm("Deseja excluir a Sugestão com Código " + cod + "?")){
        location.href='../../FUNCIONALIDADES/ADM/delete.php?cod=' + cod + '&delete=sugest';
      }
    }
  </script>
</body>
