...
<body>
  ...
  <h1>.</h1>
  <table>
    <caption>Tabela de Cadastros Pendentes</caption>
    <caption class="addLivro">
      <a href="tabelaUser.php"><button type="button" class="btn">Usuarios Cadastrados <i class="fas fa-eye"></i></button></a>
    ...
    <?php
      include('../conn.php');
      $results_per_page = 25;
      $sql = 'SELECT * FROM useranalise ORDER BY Dia';
      ...
      $sql = 'SELECT * FROM useranalise ORDER BY Dia LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
      $result = mysqli_query($conn, $sql);
      while ($linha = mysqli_fetch_array($result)) {
        echo '  <tr>
                  ...
                  <td>' . $linha['Email'] . '</td>
                  <td>' . $linha['Celular'] . '</td>
                  <td><a onclick="aceitar('.$linha['RM'].')" href="#"><i class="fas fa-check"></a></td>
                  <td><a onclick="exclui('.$linha['RM'].')" href="#"><i class="fas fa-times"></a></td>
                </tr>';
      };
      echo '</table>';
      echo '<div class="pagination">';
      for ($page = 1; $page <= $number_of_pages; $page++) {
        echo '<a href="tabelaUserAnalise.php?page=' . $page . '" class="btn">' . $page . '</a>';
      }
      echo '</div>';
    ?>
  ...
  <script>
    function exclui(cod){
      if(confirm("Deseja excluir o Aluno com o RM " + cod + "?")){
        location.href='../../FUNCIONALIDADES/ADM/email.php?rm=' + cod+'&verif=recusar';
      }
    }
    function aceitar(cod){
      if(confirm("Deseja aceitar o Aluno com o RM " + cod + "?")){
        location.href='../../FUNCIONALIDADES/ADM/email.php?rm=' + cod+'&verif=aceitar';
      }
    }
  </script>
</body>
