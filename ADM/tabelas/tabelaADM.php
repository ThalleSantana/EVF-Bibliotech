...
<body>
  ...
  <table>
    ...
    <caption class="addLivro">
      ...
      <a href="tabelaUser.php"><button type="button" class="btn">Usuarios Cadastrados <i class="fas fa-users"></i></button></a>
      <a href="../adicionar/adicionarADM.php"><button type="button" class="btn">Adicionar Administrador <i class="fas fa-plus-circle"></i></button></a>
    </caption>
    <thead>
      <tr>
        <th>Login</th>
    ...
    <?php
      ...
      $sql = 'SELECT * FROM adm LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
      $result = mysqli_query($conn, $sql);
      while ($linha = mysqli_fetch_array($result)) {
        echo '  <tr>
                  <td>' . $linha['Login'] . '</td>
                  <td>' . $linha['Nome'] . '</td>
                  <td><a href="../alterar/alterarSenhaADM.php?login=' . $linha['Login'] . '"><i class="far fa-edit"></a></td>
                </tr>';
      };
      echo '</table>';
      echo '<div class="pagination">';
      for ($page = 1; $page <= $number_of_pages; $page++) {
        echo '<a href="tabelaADM.php?page=' . $page . '" class="btn">' . $page . '</a>';
      }
      echo '</div>';
    ?>
  ...
  <script>
    function exclui(cod){
      if(confirm("Deseja excluir o Aluno com o RM " + cod + "?")){
        location.href='../../FUNCIONALIDADES/ADM/delete.php?cod=' + cod;
      }
    }
  </script>
</body>
